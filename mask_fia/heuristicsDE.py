...
timmm = 0
__all__ = ['differential_evolution'] #定义了调用该代码模块时能导出的符号（变量、函数、类等）

_MACHEPS = np.finfo(np.float64).eps  #表示应该获取float64类型的信息，eps表示正数
...
class DifferentialEvolutionSolver(object):
    ...
    def __init__(self, func, region, ct_energy,bounds, args=(),
        ...
        # relative and absolute tolerances for convergence 收敛的相对和绝对公差
        self.tol, self.atol = tol, atol  #0.01 -1

        # Mutation constant should be in [0, 2). If specified as a sequence
        # then dithering is performed.  突变常数应在[0，2]中。如果指定为序列，则执行抖动。
        ...
        self.parameter_count = np.size(self.limits, 1) # bounds里变量个数
    ...
    def init_population_lhs(self):
        ...
        # Offset each segment to cover the entire parameter range [0, 1)
                   + np.linspace(0., 1., self.num_population_members,  # 创建等差数列
    ...
    def _calculate_population_energies(self):
        ...
        #
        energies = self.func(parameters,0, *self.args)
    ...
    def __next__(self,nit):
        ...
        #[0.2 - 0.8]随迭代轮数变大
        # 
        scale_inbreeding = 0.2 + nit/self.maxiter*(0.8-0.2)
        # 
        num_inbreeding = int(scale_inbreeding * itersize)
        #uP个体产生

        param_inb,extra_set = np.array(self.region(self.parampopulation[0:num_inbreeding,:]))
        ...
        for each in extra_set:
            np.insert(trials1,each,self._mutate(each))
        trials2 = np.array([self._mutate(c) for c in range(num_inbreeding,itersize)]) # TODO: can be vectorized
    ...
    def _select_samples(self, candidate, number_samples):
        ...
        生成一代中除当前孩子以外的其余5个随机数（在一代种群数量范围之内）
        """
